...
<script>
	...
	function save(){
		...
		if(content == '' || content.match(/^\s+$/)){
			prompt('发射内容不能为空');
			//$(".alert-text").html('发射内容不能为空');
			//$("#fadebg,#reg_form").hide();
			//$("#confirm_form").attr('prompt-data','1');
...
$(function(){
	...
	$(".tel-btn").click(function(){
		...
		if(!checkMobile(phone)){
			...
			//$(".alert-text").html("亲爱的手机号好像有问题，重新检查下哟！");
			//$("#fadebg,#reg_form").hide();
			//$("#fadebg,#confirm_form").show();
			return;
		}
...
//			$("#fadebg,#confirm_form").show();
	...
	function suiji() {
		...
		if($('#play_tanmu .message-box').eq(num).attr("tag")!="me"){
			...
			//$('#play_tanmu .message-box').eq(num).find(".message").css('backgroundColor',strColor);
			$('#play_tanmu .message-box').eq(num).find(".ico-small").css("border-right-color",strColor);
		}else{
			$('#play_tanmu .message-box').eq(num).css('top',"50%");
			// $('#play_tanmu .message-box').eq(num).find(".message").css({"background-color":"red","font-size":"2rem","color":"#FFF","font-weight":"bolder"});
			// $('#play_tanmu .message-box').eq(num).find(".ico-small").css("border-right-color","red");
		}
